...
func (s *UserSubscriptionRepoSuite) TestResetDailyUsage() {
	...
	err := s.repo.ResetDailyUsage(s.ctx, sub.ID, sub.DailyWindowStart, resetAt, 0)
...
func (s *UserSubscriptionRepoSuite) TestResetWeeklyUsage() {
	...
	err := s.repo.ResetWeeklyUsage(s.ctx, sub.ID, sub.WeeklyWindowStart, resetAt, 0)
...
func (s *UserSubscriptionRepoSuite) TestResetMonthlyUsage() {
	...
	err := s.repo.ResetMonthlyUsage(s.ctx, sub.ID, sub.MonthlyWindowStart, resetAt, 0)
...
func (s *UserSubscriptionRepoSuite) TestResetDailyUsage_ToSpecifiedUsage() {
	user := s.mustCreateUser("resetd-custom@test.com", service.RoleUser)
	group := s.mustCreateGroup("g-resetd-custom")
	sub := s.mustCreateSubscription(user.ID, group.ID, func(c *dbent.UserSubscriptionCreate) {
		c.SetDailyUsageUsd(10.0)
	})

	resetAt := time.Date(2025, 3, 2, 0, 0, 0, 0, time.UTC)
	err := s.repo.ResetDailyUsage(s.ctx, sub.ID, nil, resetAt, 3.5)
	s.Require().NoError(err, "ResetDailyUsage")
...
func (s *UserSubscriptionRepoSuite) TestActiveExpiredBoundaries_UsageAndReset_BatchUpdateExpiredStatus() {
	...
	s.Require().NoError(s.repo.ResetDailyUsage(s.ctx, active.ID, after.DailyWindowStart, resetAt, 0), "ResetDailyUsage")
