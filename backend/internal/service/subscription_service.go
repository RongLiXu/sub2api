...
var (
	...
	ErrSubscriptionUsageNegative   = infraerrors.BadRequest("SUBSCRIPTION_USAGE_NEGATIVE", "subscription usage cannot be negative")
...
func (s *SubscriptionService) CheckAndResetWindows(ctx context.Context, sub *UserSubscription) error {
	...
	if sub.NeedsDailyReset() {
		...
		if err := s.userSubRepo.ResetDailyUsage(ctx, sub.ID, expectedWindowStart, windowStart, 0); err != nil {
			return err
		}
	...
	if sub.NeedsWeeklyReset() {
		...
		if err := s.userSubRepo.ResetWeeklyUsage(ctx, sub.ID, expectedWindowStart, windowStart, 0); err != nil {
			return err
		}
	...
	if sub.NeedsMonthlyReset() {
		...
		if err := s.userSubRepo.ResetMonthlyUsage(ctx, sub.ID, expectedWindowStart, windowStart, 0); err != nil {
			return err
		}
