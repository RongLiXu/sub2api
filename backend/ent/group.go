...
type Group struct {
	...
	// 订阅限额超限后是否允许使用 credit_balance 兜底；NULL 表示继承全局配置
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldSubscriptionCreditFallbackEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field subscription_credit_fallback_enabled", values[i])
			} else if value.Valid {
				_m.SubscriptionCreditFallbackEnabled = new(bool)
				*_m.SubscriptionCreditFallbackEnabled = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.SubscriptionCreditFallbackEnabled; v != nil {
		builder.WriteString("subscription_credit_fallback_enabled=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
