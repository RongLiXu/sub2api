      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button @click="$emit('clone', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-blue-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="copy" size="sm" />
              {{ t('admin.accounts.clone') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'clone', 'schedule', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
