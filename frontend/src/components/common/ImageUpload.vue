...
const uploadButtonLabel = computed(() => props.uploadLabel || t('common.upload'))
const removeButtonLabel = computed(() => props.removeLabel || t('common.remove'))
...
function handleUpload(event: Event) {
  ...
  if (props.maxSize && file.size > props.maxSize) {
    error.value = t('common.fileTooLarge', {
  ...
  if (props.mode === 'svg') {
    ...
    if (!file.type.startsWith('image/')) {
      error.value = t('common.invalidImageFile')
      input.value = ''
      return
    }
    reader.onload = (e) => {
