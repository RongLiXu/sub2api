<template>
  <AppLayout>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :disabled="cloningAccountId === row.id"
    ...
    <AccountActionMenu :show="menu.show" :account="menu.acc" :position="menu.pos" @close="menu.show = false" @clone="handleClone" @test="handleTest" @stats="handleViewStats" @schedule="handleSchedule" @reauth="handleReAuth" @refresh-token="handleRefresh" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" @create-spark-shadow="handleCreateSparkShadow" />
...
const buildCloneAccountName = (account: Account) => t('admin.accounts.cloneName', { name: account.name })
const handleClone = async (a: Account) => {
  const defaultName = buildCloneAccountName(a)
  const name = window.prompt(t('admin.accounts.clonePrompt', { name: a.name }), defaultName)
  if (name === null) return
  ...
  cloningAccountId.value = a.id
  try {
    await adminAPI.accounts.clone(a.id, { name: trimmedName })
    appStore.showSuccess(t('admin.accounts.cloneSuccess'))
    await reload()
  } catch (error: any) {
    console.error('Failed to clone account:', error)
    appStore.showError(error?.response?.data?.message || error?.response?.data?.detail || t('admin.accounts.cloneFailed'))
  } finally {
    cloningAccountId.value = null
  }
}
